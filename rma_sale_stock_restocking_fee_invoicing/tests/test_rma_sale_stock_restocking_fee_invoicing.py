...
class TestRmaSaleStockRestockingFeeInvoicing(TestRmaSaleBase):
    ...
    def test_1(self):
        """no restocking fee is applied when restocking_fee_type is 'no_fee'."""
        self.assertEqual(self.operation.restocking_fee_type, "no_fee")
        wizard = self._rma_sale_wizard(self.sale_order)
        rma = self.env["rma"].browse(wizard.create_and_open_rma()["res_id"])
        self.assertTrue(rma.reception_move_id)
    ...
    def test_5(self):
        ...
        self.assertEqual(len(self.sale_order.order_line), 1)
        self.assertTrue(rma.manual_restocking_fee_invoice_needed)
        self.assertFalse(rma.restocking_fee_invoice_id)
        rma.action_create_restocking_fee_invoice()
